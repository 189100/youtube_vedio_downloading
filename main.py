...
root.geometry('500x200')  # set up the size
color = 'gray77'
...
# https://www.youtube.com/watch?v=cLm5ko937hI
#def clear_path():
#    path.delete(0, 'end')
...
#        video.download(str(path.get().strip()))  # download the video..
        ...
        user_update = Label(bottom, font=('arial', 10), text=info, bd=5, anchor='w', bg=color)
        user_update.grid(row=0, columns=1)


# ==================================================Buttons=================
btn_clear_url = Button(right, text="Clear Url", font=('arial', 10),
                       command=lambda: clear_url(), highlightbackground=color).grid(row=1, columns=1)

#btn_clear_path = Button(right, text="Clear Path", font=('arial', 10),
#                        command=lambda: clear_path(), highlightbackground=color).grid(row=3, columns=1)

#btn_clear_all = Button(right, text="Clear all", font=('arial', 10),
#                       command=lambda: clear_all(), highlightbackground=color).grid(row=5, columns=1)

btn_down_video = Button(right,  text="Download Video", font=('arial', 10),
                        command=lambda: download_video(), highlightbackground=color).grid(row=7, columns=1)

# ==================================================Labels===============

# This is the top lable
top_lable = Label(top,  font=('arial', 10), text="YouTube Videos Downloader", bd=5,
                  anchor='w', bg=color)
top_lable.grid(row=0, columns=1)

# This is the bottom lable
bottom_lable = Label(bottom, font=('arial', 10), text="YouTube Videos Downloader", bd=5, anchor='w', bg=color)
bottom_lable.grid(row=0, columns=1)

# This is for the url
url_lable = Label(left, font=('arial', 10), text="Url: ",  anchor='w', bg=color)
url_lable.grid(row=1, columns=1)

# This is for the path
#path_lable = Label(left, font=('arial', 10), text="Path: ",  anchor='w', bg=color)
#path_lable.grid(row=2, columns=1)

# ==================================================Entry================

url = Entry(left, font=('arial', 10), width=37, bd=1, insertwidth=7)
url.grid(row=1,  column=1, sticky=E+S)

#ath = Entry(left, font=('arial', 10), width=37, bd=1, insertwidth=7)
#path.grid(row=2, column=1, sticky=E+S)

# ==================================================Radiobutton============

btn1_data = Radiobutton(left, text="Good Quality",  font=('arial', 10), variable=quality,  value=0, bg=color)
btn1_data.grid(row=0,  column=1, sticky=W)

btn2_data = Radiobutton(left, text="Bad Quality",  font=('arial', 10), variable=quality, value=1, bg=color)
btn2_data.grid(row=0,  column=1, padx=20)
